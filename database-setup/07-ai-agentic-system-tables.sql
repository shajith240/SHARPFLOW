-- AI Agentic System Database Setup - Dynamic Prompt Customization & Lead Qualification
-- Run this script in your Supabase SQL Editor

-- ============================================================================
-- COMPATIBILITY CHECK - Determine leads table ID type
-- ============================================================================
DO $$
DECLARE
    leads_id_type TEXT;
    users_id_type TEXT;
BEGIN
    -- Check leads table ID column type
    ...
    RAISE NOTICE 'Leads table ID type: %', COALESCE(leads_id_type, 'TABLE NOT FOUND');
    RAISE NOTICE 'Users table ID type: %', COALESCE(users_id_type, 'TABLE NOT FOUND');

    -- Map PostgreSQL data types to SQL types for table creation
    -- character varying -> VARCHAR, uuid -> UUID, etc.
    IF users_id_type = 'character varying' THEN
        PERFORM set_config('custom.users_id_sql_type', 'VARCHAR', false);
    ELSIF users_id_type = 'uuid' THEN
        PERFORM set_config('custom.users_id_sql_type', 'UUID', false);
    ELSE
        PERFORM set_config('custom.users_id_sql_type', 'VARCHAR', false); -- Default fallback
    END IF;

    IF leads_id_type = 'character varying' THEN
        PERFORM set_config('custom.leads_id_sql_type', 'VARCHAR', false);
    ELSIF leads_id_type = 'uuid' THEN
        PERFORM set_config('custom.leads_id_sql_type', 'UUID', false);
    ELSE
        PERFORM set_config('custom.leads_id_sql_type', 'VARCHAR', false); -- Default fallback
    END IF;

    RAISE NOTICE 'Using SQL type for users.id: %', current_setting('custom.users_id_sql_type', false);
    RAISE NOTICE 'Using SQL type for leads.id: %', current_setting('custom.leads_id_sql_type', false);
END $$;
...
DO $$
DECLARE
    users_id_type TEXT := COALESCE(current_setting('custom.users_id_sql_type', true), 'VARCHAR');
BEGIN
    EXECUTE format('
...
DO $$
DECLARE
    leads_id_type TEXT := COALESCE(current_setting('custom.leads_id_sql_type', true), 'VARCHAR');
    users_id_type TEXT := COALESCE(current_setting('custom.users_id_sql_type', true), 'VARCHAR');
BEGIN
    EXECUTE format('
    CREATE TABLE IF NOT EXISTS lead_qualification_results (
        ...
        UNIQUE(lead_id)
    )', users_id_type, leads_id_type, users_id_type);
END $$;
...
-- Insert default qualification rule types for reference
INSERT INTO lead_qualification_rules (id, user_id, rule_name, rule_type, criteria, weight, threshold, is_active, is_ai_generated, generation_reasoning)
VALUES 
    ('default-industry-match', 'system', 'Industry Match Template', 'industry_match', 
     '{"target_industries": [], "match_type": "exact_or_related"}', 0.25, 0.70, false, true,
     'Template rule for industry matching - will be customized per user'),
    
    ('default-company-size', 'system', 'Company Size Template', 'company_size', 
     '{"min_employees": 10, "max_employees": 10000, "revenue_range": "1M-100M"}', 0.20, 0.60, false, true,
     'Template rule for company size qualification - will be customized per user'),
     
    ('default-title-seniority', 'system', 'Decision Maker Level Template', 'title_seniority', 
     '{"target_levels": ["C-Level", "VP", "Director", "Manager"], "weights": {"C-Level": 1.0, "VP": 0.8, "Director": 0.6, "Manager": 0.4}}', 0.30, 0.50, false, true,
     'Template rule for decision maker level - will be customized per user'),
     
    ('default-geographic', 'system', 'Geographic Match Template', 'geographic', 
     '{"target_regions": [], "match_type": "country_or_region"}', 0.15, 0.80, false, true,
     'Template rule for geographic targeting - will be customized per user'),
     
    ('default-budget-indicators', 'system', 'Budget Indicators Template', 'budget_indicators', 
     '{"company_growth_signals": true, "recent_funding": true, "technology_adoption": true}', 0.10, 0.60, false, true,
     'Template rule for budget qualification signals - will be customized per user')
